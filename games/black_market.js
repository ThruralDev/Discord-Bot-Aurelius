...
const {databaseUri, partner_channels, channel_black_market_id} = require("../config.json")
const schemaCurrency = require("../schemas/currency")
const schemaRirisu = require("../schemas/partners/ririsu")
const schemaAzuki = require("../schemas/partners/trapmonkey")

const {channel, items, messages, embed_color} = JSON.parse(fs.readFileSync("config/black_market.json"))
let M = {
...
async function generateBalanceEmbed(_interaction) {
    let query = await schemaCurrency.findOne({playerID: _interaction.user.id}).lean()
    let embed = new MessageEmbed()
        .setColor(M.embedColor)
...
async function generateItemsEmbed(_interaction) {
    const row = new MessageActionRow()
        .addComponents(
            new MessageButton()
                .setCustomId(M.messages.buttons.partner_one.custom_id)
                .setLabel(M.messages.buttons.partner_one.label)
                .setEmoji(M.messages.buttons.partner_one.emoji)
                .setStyle(M.messages.buttons.partner_one.style),
            new MessageButton()
                .setCustomId(M.messages.buttons.partner_two.custom_id)
                .setLabel(M.messages.buttons.partner_two.label)
                .setEmoji(M.messages.buttons.partner_two.emoji)
                .setStyle(M.messages.buttons.partner_two.style))
    let embed = new MessageEmbed()
        .setColor(M.embedColor)
        .setTitle(M.messages.embeds.items.title)
        .setDescription(M.messages.embeds.items.description)
    let sizeKingslist = await schemaAzuki.countDocuments()
    let sizePantsuList = await schemaRirisu.countDocuments()
...
async function checkPosses(_obj, _user) {
    ...
    if (await schemaCurrency.findOne({playerID: _user}).lean()) {
        return true
    }
...
async function checkHasEntry(_interaction, _schema) {
    if (await _schema.findOne({playerID: _interaction.user.id}).lean()) {
        await _interaction.editReply({content: "You are already have bought this Item!"})
        return true
    }
    return false
}
                ...
                .updateOne(
                    ...
                    (err) => {
                        ...
                        let replyMessage = _item.purchase_message
                            ...
                            .replace("KLCHANNELID", partner_channels[_item.id].wl_id)
...
async function routeActions(_interaction) {
    ...
    if (_interaction.customId === "azuki") {
        let itemData = M.items[0]
        if (await checkSoldOut(_interaction, itemData, schemaAzuki)) return
        if (!await checkPosses(_interaction, _interaction.user.id)) return
        if (await checkHasEntry(_interaction, schemaAzuki)) return
        if (!await checkBalanceItem(_interaction, itemData)) return
        await purchaseRole(_interaction, itemData, "WL-Azuki", schemaAzuki)
    }

    if (_interaction.customId === "ririsu") {
        let itemData = M.items[1]
        if (await checkSoldOut(_interaction, itemData, schemaRirisu)) return
        if (!await checkPosses(_interaction, _interaction.user.id)) return
        if (await checkHasEntry(_interaction, schemaRirisu)) return
        if (!await checkBalanceItem(_interaction, itemData)) return
        await purchaseRole(_interaction, itemData, "WL-Ririsu", schemaRirisu)
    }
}

async function checkBalanceSend(_msg, _cost) {
    let query = await schemaCurrency.findOne({playerID: _msg.author.id}).lean()
    if (query.balance < _cost) {
        return false
    }
    return true
}
...
async function sendCoins(_msg, _blocks) {
    // input validation -----
    function checkRecepient(_this) {
        return _this.match(/^<@!\d+>$/g)
    }
    ...
    if (!checkRecepient(_blocks[1])) {
        _msg.reply("You have to mention your recepient with '@..'")
        return
    }
    let recepientsID = _blocks[1].replace("<@!", "").replace(">", "")
    // ----- until here
...
module.exports = (_client) => {
    ...
    _client.on("interactionCreate", async _interaction => {
        if(
            ...
            _interaction.channel.id !== channel_black_market_id
        ) return
    ...
    _client.on("messageCreate", _msg => {
        if (_msg.author.bot || _msg.channel.id !== channel_black_market_id) return
